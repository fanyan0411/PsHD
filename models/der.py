...
from utils.distill_loss import matching_loss, matching_loss_topo_samplewise,matching_loss_topo_protowise, matching_loss_topo2
...
class DER(BaseLearner):
    ...
    def after_task(self):
        ...
        if not self.args["resume"]:
            ...
            if self.args["full_supervise"]:
                checkpoints_folder_name = 'checkpoints_fullsupervise'
    ...
    def incremental_train(self, data_manager):
        ...
        if self.full_supervise:
            labeled_idxs = np.where(labeled_idxs_onehot>-100)[0]
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):  #_20241120
            ...
            for i, (_, inputs, inputs_s, targets, pse_targets, lab_index_task) in enumerate(train_loader):
                ...
                if self.topo_weight>0:    
                    ...
                    if embed_type == 'logits':
                        ...
                        loss_match_topo_class = self.topo_weight * matching_loss_topo_protowise(self._old_network(inputs)["logits"], logits, \
                                                                        self._known_classes, targets_withpse, targets, relation='mse', \
                                                                        targets_old=targets_old, rw_alpha=self.rw_alpha, T=self.rw_T, \
                                                                        gamma=self.gamma_ml, simplex_dim=self.args["toposimpl_dim"],\
                                                                        embed_type=self.args["embed_type"], class_proto_index=class_proto_index)['matching_loss_seed']
                        loss_match_topo += loss_match_topo_class
                    ...
                    elif embed_type == 'global_logits':
                        match_topo = matching_loss_topo2(self._old_network(inputs)["logits"], logits, \
                                                                                           self._known_classes, targets_withpse, targets, relation='mse', \
                                                                                           targets_old=targets_old, rw_alpha=self.rw_alpha, T=self.rw_T, \
                                                                                           gamma=self.gamma_ml, simplex_dim=self.args["toposimpl_dim"],\
                                                                                           embed_type=self.args["embed_type"])
                ...
                #print('loss_match_dsgd', loss_match_dsgd, 'loss_match_topo', loss_match_topo, 'loss_match_topo_class', loss_match_topo_class)
