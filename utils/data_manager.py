...
class DataManager(object):
    ...
    def get_dataset(
        self, indices, source, mode, appendent=None, loader_idx=None, ret_data=False, m_rate=None
    ):
        if source == "train":
        ...
        ## 
        ## Load labeled or unlabeled data
        if loader_idx == 'labeled_train':

            x = x[np.where(label_index)[0]]
            y = y[np.where(label_index)[0]]
            label_index = label_index[np.where(label_index)[0]]
        elif loader_idx == 'unlabeled_train':

            x = x[np.where(1-label_index)[0]]
            y = y[np.where(1-label_index)[0]]
            label_index = label_index[np.where(1-label_index)[0]]
        ## Load labeled or unlabeled data // end
        ...
        for idx in indices:
            ...
            lab_index_task.append(class_label_index)  #
        ...
        if appendent is not None and len(appendent) != 0:
            ...
            ## Load labeled or unlabeled data
            if loader_idx == 'labeled_train':
                appendent_data = appendent_data[np.where(appendent_targets_lab_idx)[0]]
                appendent_targets = appendent_targets[np.where(appendent_targets_lab_idx)[0]]
                appendent_pse_targets = appendent_pse_targets[np.where(appendent_targets_lab_idx)[0]]
                appendent_targets_lab_idx = appendent_targets_lab_idx[np.where(appendent_targets_lab_idx)[0]]
            elif loader_idx == 'unlabeled_train':
                appendent_data = appendent_data[np.where(1-appendent_targets_lab_idx)[0]]
                appendent_targets = appendent_targets[np.where(1-appendent_targets_lab_idx)[0]]
                appendent_pse_targets = appendent_pse_targets[np.where(1-appendent_targets_lab_idx)[0]]
                appendent_targets_lab_idx = appendent_targets_lab_idx[np.where(1-appendent_targets_lab_idx)[0]]
            ## Load labeled or unlabeled data // end
...
class DummyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print(idx, image_w.sum())

        #return idx, image_w, image_s, label, pse_labels, lab_index_task
        #print(idx, image_w.sum())
        return idx, image_w, image_s, label, pse_labels, lab_index_task #self.trsf, self.images[idx] #pse_labels, lab_index_task
